...
function MultiSelect({ titulo, opcoes, valores, onChange, comBusca = false }: { titulo: string; opcoes: { valor: string; label: string }[]; valores: string[]; onChange: (valores: string[]) => void; comBusca?: boolean }) {
  const [busca, setBusca] = useState('');
  
  const opcoesFiltradas = useMemo(() => {
    if (!comBusca || !busca) return opcoes;
    const buscaLower = busca.toLowerCase();
    return opcoes.filter(opcao => opcao.label.toLowerCase().includes(buscaLower));
  }, [opcoes, busca, comBusca]);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <MultiSelect titulo="Produtos" opcoes={opcoesFiltros.produtos} valores={filtros.produtos} onChange={(valores) => onAtualizarFiltros({ produtos: valores })} comBusca={true} />
      </div>
      <Separator />
